from torchvision.models import alexnet, vgg19, resnet18, efficientnet_b0, mobilenet_v2, mobilenet_v3_small, mnasnet0_5
...
def initialize_model(model_name, num_classes, prepth_file_path):
    ...
    model = model_funcs[model_name](pretrained=False)
    
    # Load pre-trained weights
    model.load_state_dict(torch.load(prepth_file_path, map_location=device))
...
def main():
    ...
    pth_file_path = os.path.join(pth_save_directory, f"01best_{args.model}_{args.step}_{args.gamma}_steplr_sgd_model.pth")
    png_file_path = os.path.join(pic_save_directory, f"01{args.model}_{args.step}_{args.gamma}_steplr_sgd_loss_acc.png")
    # train_model(model, device, trainloader, testloader, args, pth_file_path, png_file_path)
    model_name = args.model  #
    prepth_file_path=f'./{model_name}.pth'
    # losses = []  # 用于存储每次训练的最佳验证损失
    metrics_log = []
    for i in range(10):  # 训练模型10次
        
        model, device = initialize_model(args.model, num_classes=2, prepth_file_path=prepth_file_path)
