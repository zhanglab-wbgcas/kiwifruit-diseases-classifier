...
def train_model(model, device, trainloader, testloader, args, pth_file_path, png_file_path):
    ...
    #学习率增加的周期比例，例如 0.3 意味着前 30% 的训练周期内学习率从 base_lr 增加到 max_lr。
    #尝试不同的max_lr，通过在一个小的训练周期（几个 epoch）中从一个很小的学习率逐步增加到一个很高的学习率，
    # 并观察训练损失如何变化。 选择使损失减少最快的学习率作为 max_lr。
...
def main():
    ...
    pth_file_path = os.path.join(pth_save_directory, f"best_{args.model}_{args.step}_{args.gamma}_onelr_sgd_model.pth")
    png_file_path = os.path.join(pic_save_directory, f"{args.model}_{args.step}_{args.gamma}_onelr_sgd_loss_acc.png")
