...
def train_model(model, device, trainloader, testloader, args, pth_file_path, png_file_path):
    ...
    scheduler = StepLR(optimizer, step_size=args.step,gamma=args.gamma)  # 每10个epoch，学习率乘以0.5
...
def main():
    ...
    pth_file_path = os.path.join(pth_save_directory, f"best_{args.model}_{args.step}_{args.gamma}_steplr_adam_model.pth")
    png_file_path = os.path.join(pic_save_directory, f"{args.model}_{args.step}_{args.gamma}_steplr_adam_loss_acc.png")
